...
This is a static HTML site (no build step, `netlify.toml` publishes `.`
as-is).
...
- If another file appears necessary, stop and request approval before
  editing it.
- Never modify `netlify.toml` or GitHub configuration unless explicitly
  requested.
...
- Never run `netlify deploy --prod` — production has been deployed via CLI
  before, which bypasses git and can desync the repo from what's actually
  live. Use a GitHub pull request and its Netlify Deploy Preview instead,
  and merge only after the user gives explicit go-ahead in chat.
